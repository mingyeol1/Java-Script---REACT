...
//  객체 생성자.. 객체 생성자는 함수를 통해서 새로운 객체를 만들고 
//  그 안에 넣고 싶은 함수들을 구현할 수 있게 만들어 준다.
...
const cat = new Animal('고양이', '감자', '야옹')
...
//각 객체에 개별적으로 생성되는것이 아니라 공통의 값을 지정할 때 사용
//프로토 타입 설정 함수
...
//프로토 타입 설정 변수.
AnimalProto.prototype.shardValue = 1;


const dog1 = new AnimalProto('개' , '멍멍이','멍멍')
const cat1 = new AnimalProto('고양이', '감자', '야옹')

dog1.say();
cat1.say();

console.log(dog1.shardValue);
dog1.shardValue += 1;
console.log(cat1.shardValue);
...
AnimalInherit.prototype.shardValue = 1;
...
// 클래스
// ES5 이전에는 클래스 문법이 없었기 때문에 위에서 처럼 객체 생성자 함수를 이용했다.
// ES6 이후 부터는 class라는 문법이 추가되었다. 
class AnimalClass{
...
const catCalss = new AnimalClass('고양이','야옹이','야옹');
