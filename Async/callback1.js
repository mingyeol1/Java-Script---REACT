// 콜백은 '함수 파라미터'로 함수를 전달하여, 비동기 처리가 끝났을 때 // 콜백 특징 인자값으로 함수를 넘김.
// 전달된 함수를 실행한다. 콜백은 가독성이 좋지 못해서 유지보수 및 디버깅이
// 힘들다.
...
function register(user){        //회원 처리
    return saveDB(user, function(user){         //DB에 저장 콜백
        return sendEmail(user, function(user){  //메일 방송 콜백
...
const result = register({email:'testuser@test,com',
    password:'test1234',
    name:'testuser'});
console.log(result);